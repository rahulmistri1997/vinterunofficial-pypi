...
def test_get_all_active_symbols_async_returns_list():
    """
...
def test_get_all_active_symbols_async_with_frequency_returns_filtered_list():
    """
...
def test_get_all_active_symbols_async_symbol_only_returns_symbol_list():
    """
    Test that get_all_active_symbols_async returns a list of dicts
    """
    api_key = "my_api_key"
...
def test_get_active_asset_data():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api._get_active_asset_data(symbol="waves-usd-p-d")
        assert result == expected_output

def test_get_active_asset_data_invalid():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api._get_active_asset_data(symbol="waves-usd-p-d")

def test_get_current_rebalance_weight():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api.get_current_rebalance_weight(symbol="waves-usd-p-d")
        assert result == expected_output

def test_get_current_rebalance_weight_invalid():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_current_rebalance_weight(symbol="waves-usd-p-d")

def test_get_current_rebalance_weight_invalid_type():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
...
def test_get_contributions():
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api.get_contributions(symbol="waves-usd-p-d")
        assert result == expected_output

def test_get_contributions_invalid_type():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_contributions(symbol="waves-usd-p-d")

def test_get_contributions_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
...
def test_get_previous_rebalance_date():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api.get_previous_rebalance_date(symbol="waves-usd-p-d")
        assert result == expected_output

def test_get_previous_rebalance_date_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_previous_rebalance_date(symbol="waves-usd-p-d")

def test_get_previous_review_date():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api.get_previous_review_date(symbol="waves-usd-p-d")
        assert result == expected_output

def test_get_previous_review_date_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_previous_review_date(symbol="waves-usd-p-d")
...
def test_get_next_review_date():
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api.get_next_review_date(symbol="waves-usd-p-d")
        assert result == expected_output

def test_get_next_review_date_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_next_review_date(symbol="waves-usd-p-d")

def test_get_next_rebalance_date():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api.get_next_rebalance_date(symbol="waves-usd-p-d")
        assert result == expected_output

def test_get_next_rebalance_date_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_next_rebalance_date(symbol="waves-usd-p-d")

def test_get_next_rebalance_weight():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api.get_next_rebalance_weight(symbol="waves-usd-p-d")
        assert result == expected_output

def test_get_next_rebalance_weight_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_next_rebalance_weight(symbol="waves-usd-p-d")
...
def test_get_data_by_date():
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        result = api.get_data_by_date(symbol="waves-usd-p-d", dates="2021-01-01")
        assert result == expected_output

def test_get_data_by_date_invalid_frequency():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_data_by_date(symbol="waves-usd-p-r", dates="2021-01-01")

def test_get_data_by_time_invalid():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPI(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.Client()
    ...
    with patch.object(api.httpx_client, "get", new_callable=Mock) as mock_get:
        mock_get.return_value = Mock(json=Mock(return_value=mock_response))
        with pytest.raises(ValueError):
            api.get_data_by_time(symbol="waves-usd-p-d", start="2021-01-01")
